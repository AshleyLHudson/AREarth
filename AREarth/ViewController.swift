...
class ViewController: UIViewController, ARSCNViewDelegate {
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        // Creates a posiiton of touch and accesses first object
        let touch = touches.first
        // Gets location of the first object
        let location = touch?.location(in: sceneView)
        // Looks for real world objects that corrispond to a point
        let hitResults = sceneView.hitTest(location!, types: .featurePoint)
        // checks if we have a hit by using hit results
        if let hitTestResults = hitResults.first {
            let transform = hitTestResults.worldTransform
            let position = SCNVector3(x: transform.columns.3.x, y:transform.columns.3.y,
